"""Controlled H.4.1 reserve-balance and liquidity-condition ingestion pipeline."""
...
def build_url(series_id: str, start_date: date | None = None, end_date: date | None = None) -> str:
    ...
    if series_id not in SERIES_IDS:
        raise common.IngestionError(f"uncontrolled H.4.1 series: {series_id}")
    if start_date is not None and end_date is not None and start_date > end_date:
        raise common.IngestionError("H.4.1 start date cannot be after end date")
...
def parse_series_payload(payload: bytes, series_id: str) -> pd.DataFrame:
    """Parse and validate one H.4.1 FRED CSV series."""
    ...
    try:
        ...
        raise common.IngestionError(f"invalid H.4.1 CSV for {series_id}") from exc
    ...
    if frame.empty:
        raise common.IngestionError(f"H.4.1 series has no usable observations: {series_id}")
    if frame["observation_date"].isna().any():
        raise common.IngestionError(f"H.4.1 series has invalid dates: {series_id}")
    if not frame["observation_date"].dt.dayofweek.eq(2).all():
        raise common.IngestionError(f"H.4.1 observations must be dated Wednesday: {series_id}")
    if not frame["value"].between(0.0, 100_000_000.0).all():
        raise common.IngestionError(f"H.4.1 value outside controlled range: {series_id}")
...
def ingest(
    ...
    *,
    ...
    """Download, preserve, validate, combine and manifest H.4.1 series."""

    retrieval_time = retrieved_at or common.utc_now()
    requested = tuple(series_ids)
    if not requested or len(set(requested)) != len(requested):
        raise common.IngestionError("H.4.1 series list must be nonempty and unique")
    uncontrolled = sorted(set(requested).difference(SERIES_IDS))
    if uncontrolled:
        raise common.IngestionError(f"uncontrolled H.4.1 series: {', '.join(uncontrolled)}")

    frames: list[pd.DataFrame] = []
    raw_metadata: list[dict[str, object]] = []
    raw_directory = project_root / "data" / "raw" / "fed" / "h41"
    ...
    processed_path = (
        ...
        / "fed"
        / "h41"
        / f"h41_liquidity_conditions_{combined_digest[:12]}.parquet"
    )
    common.write_parquet_atomic(combined, processed_path)

    manifest_path = project_root / "data" / "manifests" / "h41_manifest.csv"
    for item in raw_metadata:
        common.append_manifest(
            manifest_path,
            {
                ...
                "status": "PASS",
                "notes": "H.4.1 reserve balances and liquidity conditions via official FRED CSV.",
            },
        )
...
def main(argv: Sequence[str] | None = None) -> int:
    """Command-line entry point for the H.4.1 pipeline."""
